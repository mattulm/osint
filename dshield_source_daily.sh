...
cd $SOURCES/dshield
###########################################################
wget --header="$HEADER" --user-agent="$UA22" https://www.dshield.org/ipsascii.html?limit=1000 -O dshield_ipsascii_$TODAY.txt
sleep 13;
wget --header="$HEADER" --user-agent="$UA22" https://isc.sans.edu/feeds/suspiciousdomains_Medium.txt -O dshield_domains_$TODAY.txt
sleep 13;
wget --header="$HEADER" --user-agent="$UA22" https://isc.sans.edu/feeds/suspiciousdomains_High.txt -O dshield_hostfile_$TODAY.txt
sleep 13;
wget --header="$HEADER" --user-agent="$UA22" https://isc.sans.edu/block.txt -O dshield_netblocks_$TODAY.txt
