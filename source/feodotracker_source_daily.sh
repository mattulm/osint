...
cp feodo_ipv4_$TODAY.txt /tmp/osint/ipv4/feodo_ipv4_botcc_$TODAY.txt;
#
...
cp feodo_domain_master_$TODAY.txt /tmp/osint/domains/feodo_domains_botcc_$TODAY.txt
##################################################
...
# Now the domains
while read p; do
        wget --no-check-certificate --header="$HEADER" --user-agent="$UA21" "https://feodotracker.abuse.ch/host/$p/" -O feodo_zeus_hashes_$p.html
	sleep 13;
...
cat feodo_ipv4_old.txt | sort | uniq >> feodo_ipv4_botcc_archive_$TODAY.txt
